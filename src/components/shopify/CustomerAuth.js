...
function CustomerAuth(props) {
  ...
  const createCustomerAccount = (email, password) => {
    ...
    props.customerCreate({ variables: { input } }).then((res) => {
  ...
  const loginCustomerAccount = (email, password) => {
    ...
    props.customerAccessTokenCreate({ variables: { input } }).then((res) => {
      if (res.data.customerAccessTokenCreate.customerAccessToken) {
        props.associateCustomerCheckout(
  ...
  return (
    <div
      className={`CustomerAuth ${
        props.isCustomerAuthOpen ? "CustomerAuth--open" : ""
      }`}
    >
      <button
        onClick={() => {
      ...
      <div className="CustomerAuth__body">
        <h2 className="CustomerAuth__heading">
          {props.newCustomer ? "Create your Account" : "Log in to your account"}
        </h2>
        ...
        >
          {props.newCustomer ? "Create Account" : "Log in"}
        </button>
      </div>
    </div>
  );
